...
import { 
  ...
  CheckCircle, XCircle, StopCircle
} from 'lucide-react'
...
export default function MappingPage() {
  ...
  const handleCancelAnalysis = async () => {
    ...
    try {
      ...
      setTerminalLogs(prev => [...prev, '[SYSTEM] Analysis cancelled by user.'])
    } catch (e) {
      setTerminalLogs(prev => [...prev, `[WARN] Cancel failed: ${e.message}`])
    }
  }
  ...
  const handleExport = (format) => {
    const totalMatches = realResults?.total_detections ?? (detectionMode === 'standard' ? (selectedCategory === 'vehicles' ? 142 : selectedCategory === 'people' ? 84 : 450) : 194)
    const categoryBreakdown = realResults?.category_breakdown ? JSON.stringify(realResults.category_breakdown, null, 2) : 'Not available'
    const coveragePercent = realResults?.coverage?.coverage_percent ?? 'N/A'
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 w-full">
              <GlassCard hover={false} className="p-4 border-white/5">
                ...
                <div className="rounded-xl overflow-hidden bg-black/40 aspect-video flex items-center justify-center relative">
                  ...
                  {/* AI Processing overlay — shown while analysis is running */}
                  <AnimatePresence>
                    {analyzing && (
                      <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="absolute inset-0 bg-black/75 flex flex-col items-center justify-center gap-4 rounded-xl"
                      >
                        {/* Radar pulse rings */}
                        <div className="relative flex items-center justify-center">
                          ...
                          {/* Centre spinning icon */}
                          <div className="w-12 h-12 rounded-full bg-green-500/10 border border-green-500/30 flex items-center justify-center z-10">
                            <Cpu size={22} className="text-green-400 animate-spin" style={{ animationDuration: '1.5s' }} />
                          </div>
                        </div>

                        {/* Status text */}
                        <div className="text-center">
                          <p className="text-sm font-bold text-white font-mono tracking-wider">AI INFERENCE RUNNING</p>
                          <p className="text-xs text-green-400 font-mono mt-1">YOLOv8 + ByteTrack</p>
                        </div>
                        ...
                        <div className="w-48">
                          ...
                          {etaSeconds !== null && (
                            <p className="text-[9px] font-mono text-[var(--text-muted)] mt-1 text-center">
                              ETA: ~{etaSeconds < 60 ? `${etaSeconds}s` : `${Math.ceil(etaSeconds/60)}m`}
                            </p>
                          )}
                        </div>
        ...
        <div className="xl:col-span-5 space-y-6 w-full">
          <GlassCard hover={false} className="border-cyan-500/10 p-5 space-y-6">
            ...
            <div className="pt-2 flex gap-3">
              <NeonButton 
                variant="primary" 
                className="flex-1 justify-center shadow-[0_0_20px_rgba(34,197,94,0.15)] hover:shadow-[0_0_25px_rgba(34,197,94,0.3)] transition-all py-3 font-bold uppercase tracking-widest text-xs" 
                icon={Play}
