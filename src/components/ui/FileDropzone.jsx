...
export default function FileDropzone({ 
  ...
  const validateFile = (file) => {
    ...
    if (!validTypes.includes(file.type) && !accept.split(',').some(ext => file.name.toLowerCase().endsWith(ext))) {
      setError('Invalid file format. Supported: MP4, MOV, AVI')
      return false
    }
        ...
        {!selectedFile ? (
          ...
          >
            ...
            <p className="text-base font-semibold text-[var(--text-primary)] mb-1">
              {isDragging ? 'Drop your drone footage here' : 'Upload Drone Footage'}
            </p>
            <p className="text-sm text-[var(--text-muted)] mb-3">
              Drag & drop or click to browse
            </p>
            <div className="flex items-center gap-3 text-xs text-[var(--text-muted)]">
              <span className="px-2 py-1 rounded-md bg-white/5">MP4</span>
              <span className="px-2 py-1 rounded-md bg-white/5">MOV</span>
              <span className="px-2 py-1 rounded-md bg-white/5">AVI</span>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-[var(--text-muted)]">
                  {formatSize(selectedFile.size)} • {selectedFile.type || 'video'}
                </p>
              </div>
              <button
                onClick={removeFile}
